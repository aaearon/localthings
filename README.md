...
A registry covers every model of its type, not one model. The refrigerator registry, for example, is exercised against both an RF9000-class fridge (flex zone, dual ice makers, beverage zone, cabinet lighting) and a TP1X_REF_21K (single toggle ice maker, no flex zone), and gates the entities each one doesn't have. Between them they add up to: freezer/fridge temperature sensors and setpoints, door sensors, ice maker switch, ice making status, rapid fridge/freeze, defrost delay and defrost-active status, water filter usage/status plus a reset button, self-check status/result/error with a start button, energy and power, AI energy level, vacation mode, convertible-compartment status, flex zone and beverage zone modes, cabinet lighting, Sabbath mode, autofill, and a firmware-update indicator.
